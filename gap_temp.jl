...
function findGaps(possDistV,possDistV_,endpoints,m,s)
    ...
    for j=1:numVIntervals
        #these arrays will hold the contraints each type of student provides
        #ex: ( I1 | I2  )    (  I3 )
        #   121  123   125  134   161
        #one student type is (1, 3) [book notation] or (1,0,1) [program notation]
        #to get this student type an I3 share has to be ≥ m//s - 123 = 159/246
        #and to get this student type an I3 share
        lowEnd = Array{Rational}(undef,0)
        highEnd = Array{Rational}(undef,0)
        ...
        if length(endpt)%2==0
            for i=1:Int64(length(endpt)/2)
                ...
                if lowerbound>upperbound
                        _gap=true
                        newendpoints=[newendpoints; upperbound lowerbound]
        ...
        else #has never happened but if it does, fix
            println()
            println("ERROR")
    ...
    for j=numVIntervals+1:numVIntervals + numV_Intervals
        ...
        for i=1:numV_Distributions
            if(possDistV_[i,j-numVIntervals]!=0)
                ...
                #push so that it is in the interval
                if lowerbound_temp<endpoints[j,1]
                    lowerbound_temp=endpoints[j,1]
                end
                if upperbound_temp>endpoints[j,2]
        ...
        else #this never happens but if it does I need to fix it
            println()
            println("ERROr")
            println(temp_endpt)
            println(endpt)
...
function buddymatch(endpoints, V,y,m,s)
    row,col=size(endpoints)
    i=1
    while i!=row #loop until no buddies or matches found

        row,col=size(endpoints) #get current dims
        lower = endpoints[i,2] #lower endpoint
        upper = endpoints[i+1,1] #higher endpoint
        #start assuming that the buddy for either endpoint is not
        #in endpoints already
        buddyIn_1 = false
        buddyIn_2=false

        for k=1:row
            row,col=size(endpoints)
            #find out if buddy already in endpoings
            for j=1:2
                if endpoints[k,j] == 1-upper
                    buddyIn_1=true
                end
        ...
        endpoints=reshape(endpoints,(2,Int64(length(endpoints)/2)))
        endpoints = transpose(endpoints)
        #if buddy in endpoints don't add again
        if buddyIn_1 == false && buddyIn_2 == false
            endpoints=[endpoints; [(1-upper) (1-lower)]]
            i=1 #reset to beginning
            row,col=size(endpoints)#get current dim
        end
        #sort endpoints
        endpoints=sort(collect(Iterators.flatten(endpoints)))
        endpoints=reshape(endpoints,(2,Int64(length(endpoints)/2)))
        ...
        #if in the 2 shares look for matches
        if lower>=y && V==3 #in the two shares
            matchIn_1 = false
            matchIn_2=false
            for k=1:row
